 ...
 * This function extracts the department ID, salary, number of children, spouse, and percentage contribution
 * from the query parameters and assigns the provided salary and data to all employees in the department
...
const assignSalaryToDepartment = async (req: Request, res: Response) => {
  try {
    const { departmentID, salary, children, spouse, percentage } = req.body;

    const departmentIdNumber = departmentID
      ? parseInt(departmentID as string, 10)
      : null;
    const salaryNumber = salary ? parseInt(salary as string, 10) : null;
    const childrenNumber = children ? parseInt(children as string, 10) : null;
    console.log(
      departmentIdNumber,
      salaryNumber,
      childrenNumber,
    ...
    // Check if departmenID is provided
    if (departmentID) {
      // Call the stored procedure to insert the department
      await pool.query(
        "CALL asignarSalarioDepartamento($1::SMALLINT, $2::INT, $3::SMALLINT, $4::BOOLEAN, $5::NUMERIC)",
        [departmentIdNumber, salaryNumber, childrenNumber, spouse, percentage]
      );
      // Send a success response
      res.status(200).send("Departament employees updated successfully");
    } else {
      res.status(400).send("Department id is required");
    }
  } catch (err) {
    // Log the error and send an error response
    console.error(err);
